...
int main(int argc, char* argv[]) {
  ...
  idp::Map map;
  map.populate("playing_area.map");

  // Display map contents

  for (int i = 0; i < map.points.size(); i++) {
    std::cout << "Point: " << map.points[i].name << ", x-coord: " << map.points[i].position.x << ", y-coord: " << map.points[i].position.y << ", is intersection: " << map.points[i].is_intersection << std::endl;
  }

  for (int i = 0; i < map.lines.size(); i++) {
    std::cout << "point1 is " << map.lines[i].point1 << " , point2 is " << map.lines[i].point2 << ", is straight: " << map.lines[i].is_straight << ", orientation: " << map.lines[i].orientation << std::endl;
  }
  return 0;
}
